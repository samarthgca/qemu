...
static void restore_sigcontext(CPUArchState *env, target_sigcontext *sc)
{
    int i;
    abi_ulong v;
    for (i = 0; i < 32; ++i) {
...
static void setup_sigcontext(target_sigcontext *sc, CPUArchState *env)
{
    int i;
    for (i = 0; i < 32; ++i) {
        __put_user(env->r[i], &sc->regs.gpr[i]);
    }
                    ...
                    target_sigset_t *set, CPUArchState *env)
{
    ...
    env->r[9] = default_rt_sigreturn;
    env->r[3] = sig;
    env->r[4] = frame_addr + offsetof(target_rt_sigframe, info);
    env->r[5] = frame_addr + offsetof(target_rt_sigframe, uc);
    env->r[1] = frame_addr;
